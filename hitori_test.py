...
color = [[mdl.binary_var(name="C" + str(l) + "_" + str(c)) for c in range(SIZE)] for l in range(SIZE)]

# Forbid adjacent colored cells
for l in range(SIZE):
    for c in range(SIZE - 1):
        mdl.add((color[l][c] + color[l][c + 1]) <= 1)
for c in range(SIZE):
    for l in range(SIZE - 1):
        mdl.add((color[l][c] + color[l + 1][c]) <= 1)
...
for l in range(SIZE):
    for c in range(SIZE):
        mdl.add((color[l][c] == 1) | (distance[l][c] <= MAX_DIST))
